...
import { View, Text, FlatList, Pressable, Image, ActivityIndicator, ScrollView, Platform, TextInput } from 'react-native';
...
const ChannelList = ({ onSelectChannel }) => {
  ...
  useEffect(() => {
    // Filtrar canales cuando cambia el texto de búsqueda
    const search = searchText.toLowerCase();
    const newFilteredChannels = {};

    Object.keys(channels).forEach(category => {
      newFilteredChannels[category] = channels[category].filter(channel =>
  ...
  const renderCategory = ({ item: category }) => {
    const categoryChannels = filteredChannels[category] || [];
